...
test("任一远端官方题库 SHA 不同时报告更新", async () => {
  const sources = [
  ...
  assert.equal(await hasOfficialCatalogUpdate(request, sources, RAW_BASE), true);
  assert.deepEqual(requestedUrls, [
    `${RAW_BASE}/db/%E9%A2%98%E5%BA%93%E4%B8%80.csv`,
    `${RAW_BASE}/db/%E9%A2%98%E5%BA%93%E4%BA%8C.csv`,
  ]);
});

test("全部远端官方题库 SHA 相同时不报告更新", async () => {
  ...
  const basePath = new URL(RAW_BASE).pathname;
  const request: typeof fetch = async (input) => {
    const path = decodeURIComponent(new URL(String(input)).pathname.slice(basePath.length + 1));
    return new Response(remoteCatalogs.get(path));
  };
  assert.equal(await hasOfficialCatalogUpdate(request, sources, RAW_BASE), false);
});

test("远端请求失败时由调用方静默处理", async () => {
  const request: typeof fetch = async () => new Response(null, { status: 503 });
  await assert.rejects(
    hasOfficialCatalogUpdate(request, [{ path: "db/题库.csv", sha256: "1".repeat(64) }], RAW_BASE),
    /503.*db\/题库\.csv/,
  );
});

test("未配置题库更新源时直接失败且不发起请求", async () => {
  let requested = false;
...
test("旧的单题库检查接口仍可使用", async () => {
  ...
  assert.equal(await hasDefaultCatalogUpdate(request, sha256(source), RAW_BASE), false);
});
