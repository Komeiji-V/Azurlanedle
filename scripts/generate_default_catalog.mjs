...
function readCatalog(sourcePath) {
  ...
  if (rows.length < 2) throw new Error("默认题库 CSV 为空或没有舰船数据。");
  ...
  if (names.some((name) => !name)) throw new Error("默认题库中的舰船名不能为空。");
  if (new Set(names).size !== names.length) throw new Error("默认题库不能包含重复舰船名。");
