...
/** Easy 难度的答案只从可建造（有明确建造时间）的舰船中抽取，避免全是活动限定或科研船。 */
function easyTenMatchCharacters(catalog: LocalCatalog, characters: LocalCharacter[]): LocalCharacter[] {
  ...
  if (!timerTag) return characters;
  const buildable = new Set(
    catalog.values
      .filter((value) => value.tagId === timerTag.id && /^\d{1,3}:[0-5]\d:[0-5]\d$/.test(value.value.trim()))
      .map((value) => value.characterId),
  );
  const filtered = characters.filter((character) => buildable.has(character.id));
  return filtered.length ? filtered : characters;
}
...
function createLocalGameWithAnswer(
  ...
  const answerCharacters = mode === "ten" && tenMatchDifficulty === "easy"
    ...
    : characters;
  if (!answerCharacters.length) throw new Error("Easy 难度没有可用的答案舰船。");
...
export function createSpecifiedLocalGame(
  ...
  if (!answer) throw new Error("题库中没有这位舰船，请输入完整舰船名或别名。");
...
export function getLocalAnswerName(catalog: LocalCatalog, game: LocalGame): string {
  return catalog.characters.find((character) => character.id === game.answerCharacterId)?.name ?? "未知舰船";
}
...
export function submitLocalGuess(
  ...
  if (!guessedCharacter) return { ok: false, error: "题库中没有这位舰船，请从候选列表中选择。" };

  const answer = catalog.characters.find((item) => item.id === game.answerCharacterId);
  if (!answer || !answer.active) return { ok: false, error: "答案舰船已被移除，请重新开始。" };
