...
export function AdminPanel() {
  ...
  function refresh(message?: string) {
    ...
    setNotice(message ?? `题库已载入 ${data.characters.length} 位舰船、${data.tags.length} 个标签。`);
  }
  ...
  async function selectCsv(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setNotice(`已读取 ${file.name}：${preview.rows.length} 位舰船、${preview.tagNames.length} 个标签。${same ? "标签一致，可添加或替换。" : "标签名称不同，只能替换当前题库。"}`);
    } catch (error) {
      setCsvPreview(null);
      setCsvFilename("");
  ...
  function importCsv(mode: "append" | "replace") {
    ...
    if (mode === "replace" && !window.confirm("替换会覆盖当前全部标签和舰船，确定继续吗？")) return;
    try {
      ...
      setCsvFilename("");
      if (fileInput.current) fileInput.current.value = "";
      setNotice(mode === "append" ? `已从 CSV 添加 ${csvPreview.rows.length} 位舰船。` : `已用 CSV 替换题库，共 ${csvPreview.rows.length} 位舰船。`);
    } catch (error) {
  ...
  function exportCsv() {
    ...
    link.href = url;
    link.download = `航一把题库-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
    URL.revokeObjectURL(url);
    setNotice(`已导出 ${catalog.characters.length} 位舰船的 CSV 文件。`);
  }
  ...
  function saveCsvText() {
    try {
      ...
      setNotice(`CSV 文档已保存，共 ${next.characters.length} 位舰船、${next.tags.length} 个标签。`);
    } catch (error) {
  ...
  return (
    <main className="admin-shell">
      <header className="admin-header">
        <div>
          <p className="eyebrow">Azur Lane archive</p>
          <h1>内容后台</h1>
          <p>维护舰船、别名和每一列判定标签。</p>
        </div>
          ...
          {library.catalogs.map((item) => (
            ...
            >
              <div>
                ...
                <h3>{item.name}</h3>
                <p>{item.catalog.characters.length} 位舰船 · {item.catalog.tags.length} 个标签</p>
              </div>
          ...
          {officialTable && (
            <div className="official-csv-table-wrap">
              <table className="official-csv-table">
                <caption>{editingCatalog.name} · {officialTable.rows.length} 位舰船</caption>
                <thead>
      ...
      {editorMode === "csv" ? (
        <section className="admin-card csv-document-card">
          <p className="admin-hint">直接编辑完整 CSV；保存时会校验表头、标签类型和舰船数据。</p>
      ...
      <section className="admin-card csv-card">
        ...
        <div className="csv-tools">
          ...
          <p>{csvFilename || "表头格式：舰船名、别名、启用，后续每列为“标签名（类型：类型代码）”。"}</p>
          {csvPreview && (
      ...
      <div className="admin-grid">
        <section className="admin-card">
          ...
          <p className="admin-hint">标签按题库中的定义顺序显示，反馈表、编辑器与导出的 CSV 顺序一致。</p>
          <form className="admin-form compact" onSubmit={saveTag}>
            <label>标签名称<input value={tagDraft.name} onChange={(event) => setTagDraft({ ...tagDraft, name: event.target.value })} placeholder="例如：获取方式" required /></label>
            <label>判定方式
        ...
        <section className="admin-card">
          <div className="section-title">
            <div><span>02</span><h2>{characterDraft.id ? "编辑舰船" : "添加舰船"}</h2></div>
          ...
          <form className="admin-form" onSubmit={saveCharacter}>
            ...
            <button className="admin-primary" disabled={busy}>{characterDraft.id ? "保存舰船资料" : "添加到题库"}</button>
          </form>
        </section>
      </div>
      ...
      <section className="admin-card roster">
        <div className="section-title">
          <div><span>03</span><h2>舰船题库</h2></div>
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索舰船" />
        </div>
        <div className="character-list">
          {filtered.map((character: LocalCharacter) => (
            <article key={character.id}>
              ...
              <button className="danger" onClick={() => window.confirm(`删除舰船“${character.name}”？`) && mutate({ action: "deleteCharacter", id: character.id }, "舰船已删除。")}>删除</button>
            </article>
          ))}
        </div>
      </section>
