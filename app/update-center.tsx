...
import {
  ...
  OFFICIAL_CATALOG_REPO_URL,
  OFFICIAL_CATALOG_UPDATE_CONFIGURED,
...
export function UpdateCenter() {
  ...
  const [appState, setAppState] = useState<CheckState>(APP_UPDATE_CONFIGURED ? "checking" : "unconfigured");
  const [catalogState, setCatalogState] = useState<CheckState>(
    OFFICIAL_CATALOG_UPDATE_CONFIGURED ? "checking" : "unconfigured",
  );
  ...
  async function checkCatalog(manual = false, signal?: AbortSignal) {
    if (!OFFICIAL_CATALOG_UPDATE_CONFIGURED) {
  ...
  return (
    <div className="update-row">
      ...
      <div className="update-row-actions">
        <button type="button" disabled={state === "checking" || state === "unconfigured"} onClick={onCheck}>手动检查</button>
        {state === "available" && updateUrl && <a href={updateUrl} target="_blank" rel="noreferrer">查看更新</a>}
      </div>
    </div>
  );
}
